...
files <- list.files(path = "../Data_Skin", pattern = "skin", full.names = TRUE)

#Create list to store the data
Skin_IF <- list()

#Load CCE Skin data
Skin_IF$CCE <- read_xlsx(files[1])

#Load Healthy Skin data
Skin_IF$Healthy <- read.csv2(files[2],sep = ",",check.names = F)

#Load Lupus Skin data
Skin_IF$Lupus1 <- read.csv2(files[3],sep = ";",check.names = F)
Skin_IF$Lupus2 <- read.csv2(files[4],sep = ",",check.names = F)
...
Reduce(intersect, lapply(Art_IF, colnames))#Some consensus in Ab naming
...
#Healthy
colnames(Healthy.Int) <- gsub("[-_ ].*Cell Intensity", "", colnames(Healthy.Int))

#CCE
colnames(CCE.Int) <- gsub("[-_ ].*Cell Intensity", "", colnames(CCE.Int))
...
#Complement: "C1q" "FB" "FH" "C3c" "C3d" "C5aR1" "C1r" "MBL" "C4d"
#Immune Myeloid: "CD68"
#Immune Lympho: "CD20" "CD8"
#Stroma: "CD31" "aSMA"

####5. PROCESS FLUORESCENCE INTENSITIES####
...
####6. PROCESS SUPERVISED CLASSIFICATION DATA####
...
#Healthy
colnames(Healthy.meta) <- gsub("[-_ ].*Positive Classification", "", colnames(Healthy.meta))

#CCE
colnames(CCE.meta) <- gsub("[-_ ].*Positive Classification", "", colnames(CCE.meta))
...
#Complement: "C1q" "MBL" "C3c" "C3d" "C5aR1" "C1r" "C4d" "FH" "FB"
#Immune Myeloid: "CD68"
#Immune Lympho: "CD20" "CD8" 
#Stroma: "CD31" "aSMA"
...
####7. COMBINE OBJECTS FOR SEURAT OBJECT CREATION####
...
seurat_int <- data.frame(mx_sample[,c(1:13)])#extract intensity data
...
Seurat_obj<- CreateSeuratObject(counts = t(seurat_int),
                                    meta.data = data.frame(cbind(Merged.Meta,mx_sample[c(29)]),
                                                           row.names = rownames(seurat_int)))

#Create disease status metadata
Seurat_obj$State <- Seurat_obj$slide
Seurat_obj$State[Seurat_obj$State %in% unique(grep("Lupus",Seurat_obj$slide,value = T))] <- "Lupus"

####9. Plot marker positivity across state and vasculature type#####

#C' deposits and VCAM-1 positivity

#Add artery type data to Seurat object
Seurat_obj$`Analysis Region` <- factor(Seurat_obj$Arteries)

#Rename and re-order levels for aesthetics
levels(Seurat_obj$`Analysis Region`) <- c("Papillary",
                                          "Reticular Deep",
                                          "Reticular Sup.")
Seurat_obj$`Analysis Region` <- factor(Seurat_obj$`Analysis Region`,
                                       levels=c("Papillary",
                                                "Reticular Sup.",
                                                "Reticular Deep"))
  
#Plot complement and VCAM-1 positivity and immune marker frequency per artery type (FIGURE S18D-E)
p1 <- DotPlot(Seurat_obj,
        group.by = c("State"),
        split.by = c("Analysis Region"),
        c("C1q.Pos","MBL.Pos","C3d.Pos","C3c.Pos"),
...
p2 <- DotPlot(Seurat_obj,
              group.by = c("State"),
              split.by = c("Analysis Region"),
              c("CD68.Pos","C5aR1.Pos","CD8.Pos","CD20.Pos"),
              cols = c("yellow","orange","red"),
              scale = F) +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank(),
